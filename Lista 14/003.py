...
class App:
    def __init__(self):
        ...
        self.Button1 = Button(font = 'Arial 14 bold', text = 'Consultar Disponibilidade',command=self.Consultar)
    ...
    def Consultar(self):
        self.dominio = self.Entry1.get()
        self.consulta = requests.get(f'https://brasilapi.com.br/api/registrobr/v1/{self.dominio}')
        self.dom = self.consulta.json()["status"]
        if self.dom == 'AVAILABLE':
            self.Label3.config(text='Domínio Disponível')
        else:
            self.Label3.config(text='Domínio NÃO Disponível')
